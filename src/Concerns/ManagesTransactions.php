...
trait ManagesTransactions
{
    ...
    {
        // If we don't have a session yet, or the existing session is already
        // in a transaction, create a fresh session. This prevents the
        // MongoDB driver from throwing a "Transaction already in progress"
        // error when attempting to start a new transaction on the same
        // session instance.
        ...
        // If the current session reports an active transaction, replace it
        // with a new session instance to ensure subsequent calls to
        // startTransaction() are performed on a clean session.
        try {
            if (method_exists($this->session, 'isInTransaction') && $this->session->isInTransaction()) {
                $this->session = $this->getClient()->startSession();
            }
        } catch (\Throwable $e) {
            // If the session doesn't support isInTransaction or checking fails,
            // fall back to returning the existing session.
        }
